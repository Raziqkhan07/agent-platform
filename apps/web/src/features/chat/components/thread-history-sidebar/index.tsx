...
>(({ className, open, setOpen }, ref: ForwardedRef<HTMLDivElement>) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "fixed top-0 right-0 z-10 h-screen border-l border-border bg-background shadow-lg transition-all duration-300",
              ...
              {threads.map((thread) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "flex items-center justify-between p-4 transition-all duration-300 hover:cursor-pointer hover:bg-accent",
                      isSelected
                        ? "bg-accent hover:cursor-default"
                        : "",
                    )}
